...
if [ "$BUILDKITE_PULL_REQUEST" != "false" ]; then
  ...
  echo -e "NOOP - Tests already ran for pipeline..."
  ...
  *-rc\.*)
    ./gradlew -Prelease.disableGitChecks=true -Prelease.version=$RELEASE_VERSION candidate release publish $GRADLE_SWITCHES
    ;;
  *)
    ./gradlew -Prelease.disableGitChecks=true -Prelease.version=$RELEASE_VERSION final release publish $GRADLE_SWITCHES
    ;;
  esac
else
